...
phylo.tree <- read.tree("./data/Raw_data/checklist_phylo.tree.tre")

# Extract all species that may appear in any group and prune the tree once
all_species <- data %>%
  mutate(species = str_replace_all(taxon_name, " ", "_")) %>%
  pull(species) %>%
  unique()

phylo.tree_trimmed <- keep.tip(phylo.tree, intersect(phylo.tree$tip.label, all_species)) %>%
  multi2di()

# Function: Calculate phylogenetic β diversity
get_phylo_beta_div <- function(df, status_name, all_univ) {
  ...
  # Ensure that the matrix columns match the species in the tree 
  # no longer pruning the tree repeatedly
  common_species <- intersect(colnames(mat), phylo.tree_trimmed$tip.label)
  mat_clean <- mat[, common_species, drop = FALSE]
  
  # Compute beta diversity components
  beta_result <- betapart::phylo.beta.pair(
    x = mat_clean, 
    tree = phylo.tree_trimmed, 
    index.family = "sorensen"
  )
  
  return(list(
    turnover = as.matrix(beta_result$phylo.beta.sim),
    nestedness = as.matrix(beta_result$phylo.beta.sne),
    total = as.matrix(beta_result$phylo.beta.sor),
    status = status_name
  ))
}


# Create subsets for four groups
invasive.data <- data %>% filter(invasive == 1)
non_invasive.data <- data %>% filter(non_invasive == 1)
native.data <- data %>% filter(native == 1)
all.data <- data

# Get list of all university codes
all_univ <- sort(unique(data$univ.links.uni.abbrev02))

# Compute phylogenetic beta diversity for each group
phylo_beta_invasive <- get_phylo_beta_div(invasive.data, "invasive", all_univ)
phylo_beta_noninvasive <- get_phylo_beta_div(non_invasive.data, "noninvasive", all_univ)
phylo_beta_native <- get_phylo_beta_div(native.data, "native", all_univ)
phylo_beta_all <- get_phylo_beta_div(all.data, "all", all_univ)

# Store beta diversity matrices
beta_list <- list(
  invasive_turnover = phylo_beta_invasive$turnover,
  invasive_nestedness = phylo_beta_invasive$nestedness,
  invasive_total = phylo_beta_invasive$total,

  noninvasive_turnover = phylo_beta_noninvasive$turnover,
  noninvasive_nestedness = phylo_beta_noninvasive$nestedness,
  noninvasive_total = phylo_beta_noninvasive$total,
  
  native_turnover = phylo_beta_native$turnover,
  native_nestedness = phylo_beta_native$nestedness,
  native_total = phylo_beta_native$total,
  
  all_turnover = phylo_beta_all$turnover,
  all_nestedness = phylo_beta_all$nestedness,
  all_total = phylo_beta_all$total
)

# When saving (optional)
# beta_list_dist <- lapply(beta_list, function(m) stats::as.dist(m))
# saveRDS(beta_list_dist, "./results/beta_list_phylo_dist.rds")

# When reading (optional)
# beta_list_dist <- readRDS("./results/beta_list_phylo_dist.rds")
# beta_list <- lapply(beta_list_dist, function(d) as.matrix(d))

# univ_order <- rownames(beta_list$invasive_turnover)
...
univ_order <- rownames(phylo_beta_invasive$turnover)
...
write.csv(final_results, "./results/MRM_results_phylo.csv", row.names = FALSE)
...
ggexport(final_plot, filename = "./results/plots.phylo.png",
         width = 3000,
         height = 2000,
         pointsize = 12,
         res = 300)
...
write.csv(partition_table0, "./results/MRM_phylo_Variance decomposition.csv", row.names = FALSE)
...
ggexport(vd_plot, filename = "./results/plots.phylo.vd.png",
         width = 3000,
         height = 2400,
         pointsize = 12,
         res = 300)
